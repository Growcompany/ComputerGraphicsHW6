		...
		for (int y = ymin; y <= ymax; ++y) {
			for (int x = xmin; x <= xmax; ++x) {
				...
				if (z < depthBuffer[idx]) {
					...
					// Phong shading 계산
					// 1) 퍼스펙티브 왜곡 무시한 단순 보간
					Vec3 Ppx = p0 * w0 + p1 * w1 + p2 * w2;
					Vec3 Npx = normalize(N0 * w0 + N1 * w1 + N2 * w2);

					// 2) Blinn–Phong 조명
					Vec3 L = normalize(lightPos - Ppx);
					Vec3 Vv = normalize(camPos - Ppx);
					Vec3 H = normalize(L + Vv);
					float diff = std::max(0.0f, dot(Npx, L));
					float spec = std::pow(std::max(0.0f, dot(Npx, H)), shininess);
